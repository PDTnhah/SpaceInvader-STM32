...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOC, SPI5_NCS_Pin|GPIO_PIN_2, GPIO_PIN_SET);
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
    cmd = 0; // Reset lệnh mỗi vòng lặp

    // Quét nút bấm (Nhấn xuống = LOW = GPIO_PIN_RESET do dùng Pull-up)
    if (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_2) == GPIO_PIN_RESET) {
        ...
        cmd = 3; // Nút Bắn (Nitro đổi thành Bắn)
    }

    // Nếu có nút được nhấn, đẩy lệnh vào Hàng đợi
    if (cmd != 0) {
        // Tránh nhồi nhét làm tràn bộ nhớ Queue
        if (osMessageQueueGetSpace(InputQueueHandle) > 0) {
            osMessageQueuePut(InputQueueHandle, &cmd, 0, 0);
        }
    }

    // Delay 20ms để chống dội phím (debounce) và nhường CPU cho TouchGFX vẽ đồ họa
    osDelay(20);
  }
  /* USER CODE END 5 */
}
